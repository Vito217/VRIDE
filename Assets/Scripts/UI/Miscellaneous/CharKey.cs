...
public class CharKey : VRKey
{
    public TextMeshProUGUI primaryChar;
    public TextMeshProUGUI secondaryChar;
    ...
    public override void OnClick()
    {
        try
        {
            ...
            string chr = (ShiftKey.shiftEnabled && secondaryChar != null) ? secondaryChar.text : primaryChar.text;
            if (!CapsLockKey.capsLockEnabled) chr = chr.ToLower();

            int lcp = target.caretPosition;
            int lap = target.selectionAnchorPosition;
            if (lcp != lap) DeleteSelection(ref lcp, ref lap, target);
            target.text = target.text.Insert(lcp, chr);
