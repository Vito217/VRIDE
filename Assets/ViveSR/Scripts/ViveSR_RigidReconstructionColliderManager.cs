...
{
    [ExecuteInEditMode]
    public class ViveSR_RigidReconstructionColliderManager : MonoBehaviour
    {       
        private List<ViveSR_RigidReconstructionCollider> all_colliders = new List<ViveSR_RigidReconstructionCollider>();
        private int num_clds;

        //
        private List<ViveSR_RigidReconstructionCollider> _temp_list = new List<ViveSR_RigidReconstructionCollider>();
        ...
        public float placer_interval_w = 0.1f;
        public float placer_interval_h = 0.1f;
        public float placer_up_shift = 0.03f;
        public float placer_show_scale = 0.03f;

#if UNITY_EDITOR
        public ColliderShapeType queried_shape;
        public PlaneOrientation queried_orient;
        public ColliderCondition queried_condition;
        public bool doing_query = false;
        private bool last_doing_query = false;        
        public bool doing_extract_pos = false;
        ...
        /**
        * The variable placerIntervalW has changed to placer_interval_w.
        * @warning The variable will remove in the future.
        */
        public float placerIntervalW
        {
            get { return placer_interval_w; }
            set { placer_interval_w = value; }
        }
        /**
        * The variable placerIntervalH has changed to placer_interval_h.
        * @warning The variable will remove in the future.
        */
        public float placerIntervalH
        {
            get { return placer_interval_h; }
            set { placer_interval_h = value; }
        }
        /**
        * The variable placerUpShift has changed to placer_up_shift.
        * @warning The variable will remove in the future.
        */
        public float placerUpShift
        {
            get { return placer_up_shift; }
            set { placer_up_shift = value; }
        }
        /**
        * The variable placerShowScale has changed to placer_show_scale.
        * @warning The variable will remove in the future.
        */
        public float placerShowScale
        {
            get { return placer_show_scale; }
            set { placer_show_scale = value; }
        }
#if UNITY_EDITOR
        /**
        * The variable queriedShape has changed to queried_shape.
        * @warning The variable will remove in the future.
        */
        public ColliderShapeType queriedShape
        {
            get { return queried_shape; }
            set { queried_shape = value; }
        }
        /**
        * The variable queriedOrient has changed to queried_orient.
        * @warning The variable will remove in the future.
        */
        public PlaneOrientation queriedOrient
        {
            get { return queried_orient; }
            set { queried_orient = value; }
        }
        /**
        * The variable queriedCondition has changed to queried_condition.
        * @warning The variable will remove in the future.
        */
        public ColliderCondition queriedCondition
        {
            get { return queried_condition; }
            set { queried_condition = value; }
        }
        /**
        * The variable doingQuery has changed to doing_query.
        * @warning The variable will remove in the future.
        */
        public bool doingQuery
        {
            get { return doing_query; }
            set { doing_query = value; }
        }
        /**
        * The variable doingExtractPos has changed to doing_extract_pos.
        * @warning The variable will remove in the future.
        */
        public bool doingExtractPos
        {
            get { return doing_extract_pos; }
            set { doing_extract_pos = value; }
        }
#endif        
        #endregion
        ...
        static public bool ProcessDataAndGenColliderInfo(GameObject go)
        {
            ...
            {
                ViveSR_RigidReconstructionCollider cld_info = mesh_filters[id].gameObject.AddComponent<ViveSR_RigidReconstructionCollider>();
        ...
        {
            ViveSR_RigidReconstructionCollider[] infos = GetComponentsInChildren<ViveSR_RigidReconstructionCollider>(true);
            int len = infos.Length;

            // init list with length * null
            List<ViveSR_RigidReconstructionCollider> brInfo_list = new List<ViveSR_RigidReconstructionCollider>(len);             
            List<ViveSR_RigidReconstructionCollider> cInfo_list = new List<ViveSR_RigidReconstructionCollider>(len);            
            List<ViveSR_RigidReconstructionCollider> mInfo_list = new List<ViveSR_RigidReconstructionCollider>(len);
            brInfo_list.AddRange(Enumerable.Repeat((ViveSR_RigidReconstructionCollider)null, len));
            cInfo_list.AddRange(Enumerable.Repeat((ViveSR_RigidReconstructionCollider)null, len));
            mInfo_list.AddRange(Enumerable.Repeat((ViveSR_RigidReconstructionCollider)null, len));
            ...
            for (int i = 0; i < len; ++i)
            {
                ...
                ViveSR_RigidReconstructionCollider cldInfo = infos[i];
            ...
            _LinkColliderInfo(ColliderShapeType.BOUND_RECT_SHAPE, brInfo_list, new List<ViveSR_RigidReconstructionCollider>[] { cInfo_list, mInfo_list });
            _LinkColliderInfo(ColliderShapeType.CONVEX_SHAPE, cInfo_list, new List<ViveSR_RigidReconstructionCollider>[] { brInfo_list, mInfo_list });
            _LinkColliderInfo(ColliderShapeType.MESH_SHAPE, mInfo_list, new List<ViveSR_RigidReconstructionCollider>[] { brInfo_list, cInfo_list });
        }

        private void _LinkColliderInfo(ColliderShapeType type, List<ViveSR_RigidReconstructionCollider> srcList, List<ViveSR_RigidReconstructionCollider>[] dstListArray)
        {
            for (int listID = 0; listID < dstListArray.Length; ++listID)
            {
                List<ViveSR_RigidReconstructionCollider> dstList = dstListArray[listID];
        ...
        void Awake()
        {
            ...
            ViveSR_RigidReconstructionCollider[] infoArray = GetComponentsInChildren<ViveSR_RigidReconstructionCollider>(true);
        ...
        private void _AddColliderInfo(ViveSR_RigidReconstructionCollider info)
        {
        ...
        public ViveSR_RigidReconstructionCollider GetClosestColliderWithProps(Vector3 testPos, uint[] props)
        {
            ...
            {
                ViveSR_RigidReconstructionCollider info = _temp_list[i];
        ...
        public ViveSR_RigidReconstructionCollider GetFurthestColliderWithProps(Vector3 testPos, uint[] props)
        {
            _GetAllColliderHasProps_Internal(props);    // get filtered info in tempList
            int found_id = -1;
        ...
        public ViveSR_RigidReconstructionCollider GetLargestCollider(uint[] props)
        {
            _GetAllColliderHasProps_Internal(props);    // get filtered info in tempList
            int found_id = -1;
        ...
        public ViveSR_RigidReconstructionCollider[] GetColliderByHeightRange(ColliderShapeType shapeType, float lowest_height, float highest_height)
        {
            _temp_list.Clear();
            ...
            if (lowest_height <= highest_height)
            {
                ...
                {
                    ViveSR_RigidReconstructionCollider info = all_colliders[i];
        ...
        public ViveSR_RigidReconstructionCollider[] GetAllColliderHasProps(uint[] props)
        {
            _GetAllColliderHasProps_Internal(props);
            return _temp_list.ToArray();
        }

        public ViveSR_RigidReconstructionCollider[] GetColliderWithPropsAndCondition(uint[] props, ColliderCondition condition, Vector3 testPos = new Vector3())
        {
            if (condition == ColliderCondition.NONE)
                return GetAllColliderHasProps(props);

            ViveSR_RigidReconstructionCollider info = null;            
...
#endif
        public void ShowAllColliderWithPropsAndCondition(uint[] props, ColliderCondition condition = ColliderCondition.NONE, Vector3 test_pos = new Vector3())
        {
            _temp_list.Clear();
            HideAllColliderRenderers();
        ...
        public void DrawAllExtractedPlacerLocations(ViveSR_RigidReconstructionCollider[] info_array)
        {
            for (int infoIdx = 0; infoIdx < info_array.Length; ++infoIdx)
            {
                //Vector3[] raycastPositions;
                Quaternion outRot;
                ViveSR_RigidReconstructionCollider info = info_array[infoIdx];
